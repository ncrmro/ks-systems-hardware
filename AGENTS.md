...
## Dovetail Joint System

The case uses snap-fit dovetail joints for tool-free assembly. Dovetails are defined in `modules/util/dovetail/`.

### Module Files

- `dimensions.scad` - Central parameters (angle=15°, length=20mm, height=3mm, clearance=0.15mm)
- `male_dovetail.scad` - Rail with center-slot snap-fit latch
- `female_dovetail.scad` - Boss with channel and catch recesses
- `female_dovetail_pocket.scad` - Female with support shelf for edge mounting

### Orientation Conventions

**Convention 1: Male/Female Assignment by Panel Hierarchy**

| Panel Type | Role | Dovetail Gender |
|------------|------|-----------------|
| Bottom | Base/floor | **Female** (receives connections) |
| Top | Ceiling | **Female** (receives connections) |
| Back | Vertical, attaches to base | **Male** (inserts into bottom) |
| Front | Vertical, attaches to top shell | **Male** (inserts into top/sides) |
| Side | Vertical, mixed connections | **Mixed** (male to top, female to back/front) |

**Rule**: Horizontal panels (bottom/top) are female; vertical panels (back/front) are male. Side panels use both genders depending on which edge.

**Convention 2: Latch vs Latchless by Assembly Type**

| Connection Type | Latch Setting | Purpose |
|-----------------|---------------|---------|
| Base assembly (bottom↔back) | `with_latch = true` | Semi-permanent, holds structure together |
| Top shell internal (top↔front↔sides) | `with_latch = false` | Easy separation for user access |
| External release points | `with_latch = true` | User-accessible squeeze-to-release |

**Rule**: Latched connections for structural base, latchless for removable top shell.

**Convention 3: Rotation Patterns by Edge Direction**

The dovetail modules have a default orientation where the rail/channel runs along the Y-axis. Use rotations to align with panel edges:

| Panel Edge | Rotation | Channel/Rail Faces |
|------------|----------|-------------------|
| Back edge (Y+) | `[180, 0, 0]` (female) or `[180, 0, 0]` (male) | Toward +Y |
| Front edge (Y-) | `[180, 0, 180]` (female) or `[0, 0, 0]` (male) | Toward -Y |
| Left edge (X-) | `[0, 0, -90]` (female) or `[0, -90, 0]` (male) | Toward -X |
| Right edge (X+) | `[0, 0, 90]` (female) or `[0, 90, 0]` (male) | Toward +X |
| Top edge (Z+) | `[0, 0, 0]` or `[0, 0, 90]` | Upward |

**Convention 4: Positioning Along Edges**

| Edge Type | Number of Dovetails | Positions |
|-----------|---------------------|-----------|
| Long edges (front/back) | 2 | 25% and 75% of width |
| Short edges (sides) | 1-2 | Center or 33%/67% of depth |
| Top shell connections | 2 | Varies by panel geometry |

**Convention 5: Length by Connection Type**

| Connection | Length | Rationale |
|------------|--------|-----------|
| Base assembly (bottom↔back) | 20mm (standard) | Maximum strength |
| Top shell internal | 9mm (shortened) | Adequate for removable connections |
...
### External Release Mechanism (Pico Config)

The back panel has male dovetails on its left/right edges that clip into female pockets on the side panels. Users squeeze these external release points to separate the top shell from the base assembly.

```
Side panel back edge: female_dovetail_pocket (with_catch_windows = true)
Back panel side edges: male_dovetail (with_latch = true)
```

### Code Examples

**Adding female dovetails to a bottom panel (back edge):**
```openscad
include <../../util/dovetail/dimensions.scad>
use <../../util/dovetail/female_dovetail.scad>

// Position at 25% and 75% of panel width
dovetail_positions = [width * 0.25, width * 0.75];

for (x_pos = dovetail_positions) {
    translate([x_pos, depth - (dovetail_length/2 + dovetail_clearance), wall_thickness])
        rotate([180, 0, 0])
            female_dovetail(with_catch_windows = true);
}
```

**Adding male dovetails to a back panel (bottom edge):**
```openscad
include <../../util/dovetail/dimensions.scad>
...
**Shortened dovetails for top shell (9mm length):**
```openscad
male_dovetail(length = 9, with_latch = false);
female_dovetail(length = 9, with_catch_windows = false);
```
